...
import { config } from '../../../config';
     ...
     export default class EditRelais extends Component {
      ...
      componentDidMount(){
        ...
        axios.get(config+'/BoussolePro-backend/getRelaisById.php?id='+this.props.match.params.id)
        .then(response=>{
          this.setState({
      ...
      onSubmit(e){
        ...
        axios.post(config+'/BoussolePro-backend/updateRelais.php?id='+this.props.match.params.id,EditRelais).then(res=>console.log(res.data));
        ...
        console.log(this.state);
         ...
         <Form.Item  >
          ...
          <label><b>Avec-qui souhaitez vous avoir le sprint PRO?</b><span class="required">*</span></label>
               <Input  className="input" type="text" name="name" value={this.state.AvecQui} onChange={this.onChangeAvecQui} placeholder="Nom du relais" required/>
        </div>
        </Form.Item  >
        <Form.Item  >
         <div class="item">
